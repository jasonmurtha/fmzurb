...
Displays the HTML inside the helper if the value is defined. 
...
The opposite of `#if`, `#unless` will only display the HTML inside the helper if the value is not defined.
...
```
// Example file src/helpers/year.js
module.exports = function() {
  return new Date().getFullYear();
}
```

Then in your projects call your custom `{{year}}` helper
...
Panini will load in the contents of this YML file as a variable called `{{ breakfast }}`. Because it's an array, we can loop through it using Handlebars's `{{#each}}` helper, and reference the item using either `{{this}}` or `{{.}}`:
